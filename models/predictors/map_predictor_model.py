...
class OccupancyPredictor(nn.Module):
    ...
    def forward(self, batch, is_train=True):
        ...
        B, C, cH, cW = step_ego_crops.shape # batch, sequence length, _, crop height, crop width
        # import pdb; pdb.set_trace()
        # (batch, num_classes, crop_height, crop_width)
        ...
        # pred_maps_raw_spatial = pred_maps_raw_spatial.view(B,spatial_C,cH,cW)
        pred_maps_spatial = F.softmax(pred_maps_raw_spatial, dim=1)
    ...
    def loss_cel(self, batch, pred_outputs):
        ...
        B, spatial_C, cH, cW = pred_maps_raw_spatial.shape

        gt_crops_spatial = batch['gt_grid_crops_spatial'].long()
        # import pdb; pdb.set_trace()
        pred_map_loss_spatial = self.cel_loss_spatial(input=pred_maps_raw_spatial.view(B,spatial_C,cH,cW), 
                                                      target=gt_crops_spatial.view(B,cH,cW))
